...
describe("Admin | Users | Change lists", () => {
    beforeEach(async () => {
        //get auth token
    ...
    it("Add list", (done) => {
        agent
            .patch(
                `${APIS.admin.users.change.lists.url}/lavr.marudenko@gmail.com/add/testMocha`,
            )
            .set("cookie", `access_token=${config.access_cookie}`)
            .end((err, res) => {
                res.should.have.status(200);
                res.body.msg.should.be.equal(
                    "List testMocha added successfully.",
                );
                done();
            });
    });
    ...
    it("Delete list", (done) => {
        agent
            .patch(
                `${APIS.admin.users.change.lists.url}/lavr.marudenko@gmail.com/delete/testMocha`,
            )
            .set("cookie", `access_token=${config.access_cookie}`)
            .end((err, res) => {
                res.should.have.status(200);
                res.body.msg.should.be.equal(
                    "List testMocha deleted successfully.",
                );
                done();
            });
    });
