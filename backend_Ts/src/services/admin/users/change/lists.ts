...
export const users_change_lists = async (req: Request, res: Response) => {
	try {
		...
		const query = await pool.query(
			`UPDATE Users SET user_list = ${move}(user_list, $1) WHERE user_email = $2`,
		...
		return res.status(200).json({
			msg: `List ${req.params.value} ${msg} successfully.`,
		});
	} catch (error) {
